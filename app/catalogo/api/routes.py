...
def get_categorias(session: Session = Depends(get_session)):
    try:
        from sqlmodel import text as sql_text
        result = session.execute(sql_text("SELECT CategoriaID, NombreCategoria FROM CATEGORIAS")).fetchall()
        return [{"CategoriaID": row[0], "NombreCategoria": row[1]} for row in result]
    except Exception as e:
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
