...
# RETORNA LA LISTA DE RESTAURANTES MAQUETADA PARA EL FRONTEND
def get_restaurants_list(session: Session) -> List[Dict]:
    restaurantes = get_all_restaurants(session)
    formatted = []
    for r in restaurantes:
        formatted.append({
            "id": r.Slug,
            "name": r.NombreRestaurante,
            "tagline": r.Tagline,
            "rating": 4.9,  # CALIFICACION POR DEFECTO ESTANDAR DE DISEÑO
...
# RETORNA LA LISTA DE PRODUCTOS CON DETALLES DE RESTAURANTE COMBINADOS
def get_dishes_list(session: Session) -> List[Dict]:
    query = text(
        "SELECT p.ProductoID, p.NombreProducto, p.Descripcion, p.ImagenURL, p.Precio, p.Stock, p.Tag, p.BadgeText, p.SpicyLevel, p.Sizes, p.Extras, "
        "       r.Slug AS RestaurantSlug, r.NombreRestaurante AS RestaurantName "
        "FROM PRODUCTOS p "
    ...
    for row in result:
        ...
        dishes.append({
            ...
            "category": row[11],
            "restaurant": row[12],
            "rating": 4.8,  # RATING ESTANDAR MOCK DE PLATILLO
            "badgeText": row[7],
            "tag": row[6],
            "spicyLevel": row[8],
            "imageSrc": row[3],
...
def add_dish(session: Session, data: Dict) -> Dict:
    ...
    new_product = Producto(
        SucursalID=suc_id,
        CategoriaID=1,  # GENERAL CATEGORIA ID
