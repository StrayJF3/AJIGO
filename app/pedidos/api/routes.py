...
from schemas.pedidos import OrderCreateRequest, StatusUpdateRequest, DriverAssignRequest, CartSaveRequest
...
@router.get("/carrito")
def get_user_cart_route(email: str, session: Session = Depends(get_session)):
    try:
        from domain.service import get_user_cart
        return get_user_cart(session, email)
    except ValueError as e:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail=str(e)
...
# RUTA PARA GUARDAR/ACTUALIZAR EL CARRITO DE UN USUARIO
@router.post("/carrito")
def save_user_cart_route(req: CartSaveRequest, session: Session = Depends(get_session)):
    try:
        from domain.service import save_user_cart
        return save_user_cart(session, req.email, req.items)
    except ValueError as e:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail=str(e)
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Error al guardar el carrito: {str(e)}"
        )